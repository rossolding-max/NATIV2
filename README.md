...
- `docs/data_lineage.md` — Complete wiring diagram for the system. Four sections: (§1) phase-to-phase flow graph cataloguing ~110 cross-phase data flows with concrete schema paths grouped by consumer phase; (§2) per-phase IO catalog covering all 12 phases + auto-archive with inputs (with source paths) + outputs (with destination paths) + computed fields (with derivation rules); (§3) reverse field index for high-traffic / cross-phase fields documenting writers + readers; (§4) gap analysis surfacing 16 identified issues (2 critical multi-tenancy gaps deferred to v2 with conscious acknowledgement + 8 high-priority deferred to v0.2 + 6 cosmetic future-proofing). Confirms no broken runtime dependencies — every consumer's required input is produced somewhere upstream.
