...
- `docs/dev_setup.md` — Quickstart for new contributors. Prerequisites (Docker + uv + Python 3.12 + Node 20 + pnpm + just + optional direnv); 12-step bootstrap (clone → .env → just up → just install → just migrate → just seed → just dev-api/worker/beat → just test); common dev workflows (add endpoint / add DB table / add Celery task / run phase walkthroughs / re-gen Pydantic from JSON Schemas / inspect LLM costs); troubleshooting table (7 common failure modes + fixes); IDE setup (VS Code + PyCharm extension lists + .vscode/settings.json); target repo layout post-M0; reading order guide ("you want to X → read Y").
