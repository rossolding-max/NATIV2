...
> 👋 **New here?** Start with [`docs/build_kickoff.md`](docs/build_kickoff.md) — orientation memo for the dev team picking up the build. Read it before anything else.
...
### Output
A single agency-identity record at `data/agency_profile.json` (gitignored — contains real agent contact details, mailbox IDs, branding asset URLs). Validated against `schemas/agency_profile.schema.json`. Prerequisite to all talent onboarding + every downstream artefact (every deck, contract, invoice references the agency identity + branding captured here).

### 9-step setup
| Step | What |
|---|---|
| 1 | **Agency identity** — name, domain, website, CAN-SPAM physical address |
| 1.5 | **Visual branding** — logo + dark logo + primary/secondary/accent colors + heading/body fonts + google_fonts list + tagline. Feeds every downstream artefact's renderer (Phase 4.5/4.6/4.7/4.8/4.9). Rebrand once → propagates to all future generations on next render. |
| 2 | **Primary agent** — agent_id slug, name, email, phone, title, LinkedIn (v0.1 maxItems:1 enforced) |
| 3 | **DNS records** — SPF + DKIM + DMARC TXT records on agency domain; Smartlead verifies each via API before proceeding |
| 4 | **Sending mailbox provisioning** — typically `{agent_first_name}@{agency_domain}`; Smartlead onboards + returns mailbox_id; daily_send_cap starts at 50 ramping to 200 post-warmup |
| 5 | **Signature template** — CAN-SPAM-compliant (must include physical address); supports `{agent_name}`/`{agency_name}`/`{unsubscribe_link}` merge fields |
| 5.5 | **Invoice template + commission defaults** — agency-wide invoice markdown + numbering format (`INV-{YYYY}-{seq:04d}`) + atomic sequence counter + tax_handling enum + payment instructions + invoice footer. Plus `default_commission_rate` (decimal, default 0.20) + `default_commission_model` enum (default `agency_invoices_brand_pays_talent_net`) — drives Phase 4.8 FROM-party determination. |
| 6 | **Mailbox warmup** — Smartlead peer-to-peer warmup runs 2-4 weeks in background; no user action required |
| 7 | **Final validation + save** — schema validation; transition `data/agency_profile.json` status from `draft` → `active`; agency is ready to onboard talent |

### State machine
```
init → draft (steps 1-5.5 in progress)
     → dns_pending (Step 3 DNS records added; waiting Smartlead verify)
     → mailbox_provisioning (Step 4 in progress)
     → warming_up (Step 6 background warmup, 2-4 weeks)
     → active (Step 7 validated; talent onboarding enabled)
     → suspended (manual: paused outreach; talent onboarding still allowed)
```

### Single-agent constraint (v0.1)
`agents` and `sending_mailboxes` arrays enforce `maxItems: 1` in v0.1. v2 lifts the constraint to enable multi-agent rosters (e.g. Sarah handles 5 talents, Tom handles 3) — schema is structurally ready (array-of-objects); only the constraint lifts.

### Integration touchpoints
| Downstream consumer | What it pulls from Phase 0 |
|---|---|
| **Phase 3b outreach** | `sending_mailboxes[0]` + `default_signature_template` for outbound email |
| **Phase 4.5 discovery prep** | `branding` for every slide; agency identity for cover slide |
| **Phase 4.6 proposal pack** | Same + `agency_process` slide content |
| **Phase 4.7 contract pack** | `billing_entity` for agency party identification; agency address for governing-law / venue clauses |
| **Phase 4.8 invoice pipeline** | `invoice_template` + `default_commission_rate` + `default_commission_model` for FROM-party determination + numbering atomic counter |
| **Phase 4.9 performance report** | `branding` for report cover + footer |

### Vendor stack
**Smartlead** for mailbox provisioning + warmup + outbound sending. Env vars: `SMARTLEAD_API_KEY`. See `docs/vendor_roadmap.md`.
...
### Files
- `schemas/agency_profile.schema.json` — JSON Schema for the Phase 0 agency identity. Captures: agency name/domain, `branding` block (logo + colors + fonts — feeds Phase 4.5/4.6/4.7/4.8/4.9 renderers), `agents[]` (v0.1 maxItems:1; v2 multi-agent), `sending_mailboxes[]` + Smartlead warmup state, CAN-SPAM-required company address, `default_signature_template` (with merge fields for `{agent_name}`/`{agency_address}`/etc.), `billing_entity` (agency's own legal entity, distinct from talent.billing_entity), `invoice_template` (Phase 4.8 — markdown source + numbering format + sequence counter + tax_handling enum + payment instructions + invoice footer — agency-wide; talent-specific overrides via talent.invoice_payment_override), `default_commission_rate` + `default_commission_model` enum (Phase 4.8 — drives FROM-party determination on brand-facing invoices; 3-value enum: agency_invoices_brand_pays_talent_net default / talent_invoices_brand_agency_invoices_talent / talent_invoices_brand_talent_pays_agency). v0.1 enforces single-agent constraint via `agents` `maxItems: 1`. Validates `data/agency_profile.json` (gitignored).
- `schemas/deal.schema.json` — JSON Schema for Phase 4 deal pipeline records. 5-stage lifecycle (LEAD → PROPOSAL → CONTRACT → DELIVERY → CLOSE) + 31-value substage enum + state machine + per-stage data blocks + attachments + notes + 31-value substage enum + structured loss-reason enum. Top-level: `originating_enrollment_id` + `originating_decision_role_at_pitch` (bidirectional with pitch_enrollment.created_deal_id; anchors won-by-role analytics). Per-stage blocks: `lead` (discovery_call notes + brief + qualification + Phase 4.5 prep_pack FKs + Phase 4.6 discovery_debrief — 10 structured fields from hybrid extraction with confidence levels); `proposal` (Phase 4.6 proposal_pack FKs + commercial_confirmed gate + canonical deliverables/fee/usage_rights/exclusivity/timeline + negotiation_log[].proposal_pack_version bidirectional links); `contract` (Phase 4.7 contract_pack FKs + legal_reviewer_id + signed timestamps + amendment_log[].contract_pack_version bidirectional links); `delivery` (content_drafts + posting_schedule[] with posted_detection block for Phase 4.8 detection provenance + campaign_hashtags[] for matching + interim_kpi_snapshots[] for Phase 4.9 daily KPI cron writes); `close` (Phase 4.8 invoice_schedule[] LLM-parsed from contract.payment_terms with one-time agent gate + invoice_pack_ids[] + all_invoices_paid_at + Phase 4.9 performance_report_pack_ids[] + final_kpis + final_performance_report_attachment_id — all three required for auto-archive). debriefSignal $def shared across debrief fields. Validates files under `data/deals/` (gitignored — commercial data + contracts + invoice amounts).
...
- `schemas/memo.schema.json` — JSON Schema for the agent memo store (cross-invocation, cross-deal, cross-brand, cross-industry memory). 5-value scope enum (deal_specific / brand_relationship / industry_pattern / talent_pattern / cross_cutting) + 10-value memo_type enum (deal_summary / brand_observation / negotiation_pattern / kpi_pattern / objection_handler / creative_insight / talent_learning / industry_insight / agent_decision_audit / other). Tag system (talent_ids[] / brand_ids[] / industry_ids[] / deal_ids[] / topics[] / phase_context) drives deterministic tag-filter retrieval via the `read_memos` tool (no vector embedding in v0.1; pgvector deferred to v0.2). Lifecycle fields: supersedes_memo_id chain + expires_at TTL + is_active soft-delete + retrieval_count + last_retrieved_at audit. Validates `memo` rows in Postgres + (v0.2) S3-offloaded large memos.
- `schemas/talent.schema.json` — JSON Schema (Draft 2020-12) describing the talent profile. Captures: identity (id + name + pronouns + contact + billing_entity with legal name/tax_id/payment_methods), `platforms[]` with per-platform stats + `api_credentials` for Phase 4.8 detection + 4.9 KPI capture oAuth tokens (reference-only — never raw secrets), aggregate + per-platform `audience_demographics` in IAB 13-band taxonomy, `previous_brands[]` lightweight index (rich record in Phase 1.5 brand_deals file), `similar_talent[]` for positioning, `rate_card`, `brand_preferences` (blocked industries, preferred industries, values red-lines, active exclusivities), `working_terms` (default usage rights, turnaround, revisions, blackout dates), `contract_template` (Phase 4.7 — per-talent markdown source + merge_field_definitions[] + clause_applicability_rules[] + narrative_placeholders[] + governing law defaults + legal_reviewer_id + template version; captured at Phase 1 onboarding Step 7.5), `commission_override` (Phase 4.8 — per-talent commission_rate + commission_model enum overriding agency defaults), `invoice_payment_override` (Phase 4.8 — payment_instructions + preferred_payment_method for talents whose deals route to their own account), `press_kit`, `disclosure_defaults` (FTC/ASA tag style; wired into contract template). Validates files under `talents/{id}.json`.
...
- `docs/build_kickoff.md` — **Orientation memo for dev team picking up the build.** Read first. 13 sections: TL;DR + product in 60s + where we are now + 6 mandatory pre-build docs to read + read-before-coding triggers + locked decisions (don't relitigate) + first-week plan + critical rules + common pitfalls + where the v2 stuff lives + where to ask questions + 12-question self-test + pre-PR checklist. Designed to take ~15 min to read; gets a new dev oriented enough to start M0 within a day.
- `docs/spec_methodology.md` — **Meta-document: the methodology used to produce this entire repo.** Generic SOP for taking a project from concept to build-ready spec via structured AI-assisted Q&A. Documents the 10 phases (domain spec foundation → architecture interview → architecture + project plan synthesis → data lineage audit → gap remediation → test plan → dev-readiness audit → conventions + hygiene → simplification audit → build kickoff memo) with: per-phase goal + inputs + outputs + template prompts + time estimates + "done when" criteria + NATIV2 worked example. Includes anti-patterns to avoid + final "ready to build" checklist + quick reference card + adapting-to-smaller-projects + adapting-to-existing-codebases guidance. Reusable for any future project.
- `docs/architecture.md` — Locked backend architecture spec. Stack (Python/FastAPI/Celery/Postgres/Redis/S3/Claude Agent SDK), system component diagram, agent topology (coordinator + 4 skill specialists: researcher/writer/extractor/renderer), LLM tier map (Opus 4.7 everywhere in v0.1), memo store retrieval mechanics, hybrid context loading model (per-pack bundle composition + augment tools), Postgres schema strategy + pgcrypto secrets + S3 storage layout + Redis usage, Celery queue catalog + 10 scheduled cron jobs, phase skip-handling matrix, vendor wrapper inventory + webhook receivers, Sentry + Langfuse instrumentation patterns, schema-to-code pipeline (auto-gen Pydantic + hand-write SQLAlchemy), and target repo layout.
- `docs/project_plan.md` — Dependency-ordered milestones (M0-M17). Foundation first (repo skeleton, data layer for all 15 schemas, agent infrastructure, vendor wrappers), then phases sequentially with explicit handoff + interdependency checks at each boundary. M11 (Phase 4.5 prep pack) is the first AI pack and validates the entire agent architecture. Cross-cutting concerns: testing discipline, memo store discipline per agent milestone, schema migration discipline, observability discipline, skip-handling discipline. v0.1 completion criteria + v2 prep (M17) defined.
- `docs/data_lineage.md` — Complete wiring diagram for the system. Four sections: (§1) phase-to-phase flow graph cataloguing ~110 cross-phase data flows with concrete schema paths grouped by consumer phase; (§2) per-phase IO catalog covering all 12 phases + auto-archive with inputs (with source paths) + outputs (with destination paths) + computed fields (with derivation rules); (§3) reverse field index for high-traffic / cross-phase fields documenting writers + readers; (§4) gap analysis — 9 fixed in repo + 1 reclassified as by-design + 6 explicitly deferred to v0.2. Confirms no broken runtime dependencies — every consumer's required input is produced somewhere upstream.
- `docs/test_plan.md` — Comprehensive backend test architecture. Two test phases: (1) automated suite running in CI on every PR + nightly against synthetic fixture (Acme Talent Agency + Riley Carter); (2) hands-on CLI test session with real agency + real talent via `nativ test ...` tool (real fixtures live local-only at `~/.nativ/test_fixtures/`). 100% coverage target with metric-per-module-type (line coverage for imperative code; LLM eval pass rate for agent code; transition coverage for state machines; contract completeness for APIs). 12 sections: test strategy + pyramid + categories; per-milestone test deliverables (M0-M17); per-app-phase test deliverables (P0-P4.9 + auto-archive) with 4 layers each (unit / integration / e2e / LLM eval); `nativ test` CLI tool design with interactive phase walkthroughs; LLM eval cassette infrastructure (vcrpy + nightly real-LLM gated by budget); synthetic fixture spec + real-data manual testing protocol + safe-by-default sends discipline; OpenAPI contract tests + stub frontend smoke (Vue 3 + Vite + Playwright headless wiring up critical UI flows against `/openapi.json`); CI pipeline structure (PR gate <8min / nightly <30min ~$10-25 LLM cost / pre-release <2hr).
- `docs/auth_and_authorization.md` — v0.1 = no auth (single-tenant local-hosted; FastAPI binds to 127.0.0.1 with defensive opt-in to bind elsewhere; agency_id + agent_id come from singleton rows in DB). v2 plan = JWT/cookie with provider TBD (Clerk / Supabase / custom / passwordless candidates documented); cross-agency isolation enforced at 3 layers (middleware + repository + Postgres RLS). Webhook signature validation per-vendor (Smartlead HMAC-SHA256, Meta `X-Hub-Signature-256`, TikTok HMAC, Stripe-Signature). Repository-layer agency_id filtering hardens from day 1 so v1→v2 needs no domain-code rewrite.
- `docs/api_conventions.md` — Every endpoint follows: `/api/v1/...` URL prefix; always-envelope responses `{data, meta, errors}` with cursor-based pagination meta; error taxonomy with SCREAMING_SNAKE_CASE codes; long-running ops via 202 + task-id + polling at `/api/v1/tasks/{id}` (status enum queued/running/completed/failed/cancelled); Idempotency-Key header (Stripe pattern) on all writes with 24h-default TTL (7d for pack-gen kickoffs); If-Match optimistic concurrency on versioned resources (all pack types, deal, talent, agency_profile); standard headers (X-Request-Id, ETag, Retry-After, X-LLM-Cost-USD); server-side ID generation; status code conventions; OpenAPI tags per Phase; CORS deferred to v2; contract test enforcement (CI requires every registered route has a test).
- `docs/id_conventions.md` — Every ID is server-generated. Per-record-type format table covering 25+ record types: UUIDs for agency_id; kebab-slugs for human-named entities (talent / brand / agent / template) with collision-suffix; prefixed nanoids for ephemeral records (memo / pe / con / pa / etc.) with 71 bits of entropy at length 12; composite prefixed IDs for time-anchored packs (`prep_YYYYMMDD_nanoid`, `inv_YYYYMMDD_seqN_vM_nanoid`, `bd_YYYY_qN_brand-slug_nanoid`); uniqueness scope per ID type (some agency-scoped via composite UNIQUE, some global); single source of truth for ID generation in `app/utils/ids.py` with one function per ID type; defensive validation via Pydantic Field patterns + DB CHECK constraints (defence in depth).
- `docs/code_conventions.md` — Python toolchain (3.12 + uv + ruff + pyright strict + pytest + alembic); ruff config (line-length 100; DTZ for tz-aware datetime enforcement; ASYNC for blocking-in-async detection; S for security; ANN for annotation requirement); naming conventions (snake_case modules / PascalCase classes / SCREAMING_SNAKE_CASE constants / test_unit__scenario__expected); async discipline (no mixing sync+async in service layer); custom exception hierarchy (NATIV2Error → ValidationError / BusinessRuleError / NotFoundError / ConflictError / IntegrationError / DegradedModeError / AuthorizationError with HTTP status mapping); structlog conventions (mandatory context keys: request_id, agency_id, agent_id, talent_id, deal_id, pack_id, task_id, vendor; PII forbidden in logs); soft-delete pattern (system-wide via SoftDeleteMixin); Decimal everywhere for money + multi-currency via fee_original_amount + fee_usd dual fields; UTC + ISO 8601 everywhere; no-comments-by-default discipline; test conventions (test_unit__scenario__expected naming + factory_boy + parametrize + pytest markers); 14-item don't-do anti-pattern list.
- `docs/configuration.md` — Full env var inventory (~70 variables) via pydantic-settings model. Per-environment differences (development / test / production); .env.example committed with safe defaults + clear comments + empty fields for secrets (Postgres encryption key, vendor API keys, Sentry DSN, etc.); secrets list that must NEVER be committed (with trufflehog enforcement); startup validation discipline (refuse to bind 0.0.0.0 without explicit opt-in; fail fast on missing required secrets); SecretStr usage pattern to prevent accidental logging.
- `docs/dev_setup.md` — Quickstart for new contributors. Prerequisites (Docker + uv + Python 3.12 + just + optional direnv; Node/pnpm not required in v0.1 since stub frontend was deferred to v2); bootstrap (clone → .env → just up → just install → just migrate → just seed → just dev-api/worker/beat → just test); common dev workflows (add endpoint / add DB table / add Celery task / run phase walkthroughs / re-gen Pydantic from JSON Schemas / inspect LLM costs); troubleshooting table; IDE setup; target repo layout post-M0; reading order guide.
- `docs/v2_deferred_requirements.md` — Single source of truth for everything v0.1 explicitly defers to v2. 11 categories: auth + multi-tenancy (V2-AUTH-01 to 05), API protocol features (V2-API-01 to 05: optimistic concurrency, cursor pagination, full idempotency, CORS, per-agency rate limits), storage + uploads (V2-STORAGE-01 to 05: presigned PUT, lifecycle policies, antivirus, pgvector memo, S3 offload), concurrency + scale (V2-SCALE-01 to 04: 4-queue Celery, multi-process FastAPI, read replicas, per-agency LLM budgets), agent + pack features (V2-PACK-01 to 07: prep pack slide rendering, sibling commission auto-gen, closed-loop pitch_angles, industry KPI benchmarks, more cassette scenarios, cancellation, debouncing), frontend readiness (V2-FRONT-01 to 03: stub frontend / SSE streaming / typed client autogen), vendor integrations (V2-VENDOR-01 to 06: Stripe + DocuSign + Phyllo + others), data layer (V2-DATA-01 to 08: pitch_history deprecation, formal schemas for static data, talent.id agency-scoping, audience demographics snapshot store, kpiMetric extraction, deleted_by_agent_id enforcement, per-agency starters), observability (V2-OBS-01 to 05: Sentry prod, Langfuse cloud, LLM dashboards, audit log table, perf dashboards), deployment + ops (V2-OPS-01 to 05: hosting decision, backup + DR, GDPR purge tool, KMS, multi-environment). Each entry includes cost estimate + v0.1 hardcoded behaviour to replace. Capability matrix at end maps every feature to v0.1-yes/no vs v2-yes/no.
- `CLAUDE.md` — Repo-level AI assistant context. What this codebase is + where to look first + locked architectural decisions table (don't relitigate); strict don'ts (no os.environ in domain code / no naive datetime / no float for money / no PII in logs / no Any type hints / no hard-delete via API / etc.); strict dos (read workflow docs before changing schemas / tests in same PR / version bumps on template changes / Idempotency-Key on writes / etc.); schema-change discipline (8-step checklist); agent-code discipline (coordinator + 4 skill subagents + memo tag-filter retrieval + prompt caching + cost tracking + cassette-first tests); test discipline; where AI assistants help most vs should be careful.
- `CONTRIBUTING.md` — Branch naming (`feature/` / `fix/` / `chore/` / `refactor/` / `test/` / `spec/`); commit message format with WHY-not-WHAT body + worked examples; PR template + review requirements (2 reviewers for schema/auth/security/conventions changes, 1 for others); merge requirements (squash-merge default; rebase-merge for multi-commit PRs); code review tone + author/reviewer responsibilities; SemVer + release process; issue reporting (security goes to SECURITY.md, not public issues).
- `SECURITY.md` — Vulnerability disclosure policy. Supported versions table; private reporting via security@mercandco.com (or GitHub private vuln reporting); SLA per CVSS severity (Critical 24h ack / 72h fix); in-scope vs out-of-scope; security practices summary (secrets never committed via trufflehog; pgcrypto AES-256 at rest; webhook HMAC validation; LLM prompt injection mitigations via schema validation + hard gates); CVSS 3.1 classification.
- `.editorconfig` + `.pre-commit-config.yaml` + `.markdownlint.yaml` — Cross-IDE indent/encoding consistency + aggressive pre-commit hooks (basic file hygiene + ruff lint+format + pyright strict + trufflehog secret scan + schema codegen drift + JSON Schema validation + unit tests for changed files + conventional commit message check + markdown lint) + permissive markdown lint config for spec docs.
- `.env.example` — Every env var enumerated (~70 vars) with safe defaults + clear comments + empty fields for secrets (Postgres encryption key, vendor API keys, Sentry DSN, etc.). Copy to `.env` and fill in secrets per `docs/configuration.md`.
- `docs/contract_template_examples/generic_starter.md` — Comprehensive 16-section example contract template scaffold demonstrating all three placeholder types the Phase 4.7 contract pack pipeline recognises: deterministic `{{merge_field}}`, conditional `{{#if condition}}…{{/if}}`, narrative `{{narrative_*}}`. Includes detailed adoption notes mapping each placeholder type to the `talent.contract_template` configuration fields it needs (merge_field_definitions / clause_applicability_rules / narrative_placeholders). Explicitly disclaimed as not legal advice; agencies must lawyer-review + adapt to their jurisdiction. Eliminates cold-start friction for agencies authoring their first contract template (data lineage GAP-10 fix).
