...
| Artefact | Generated from | Renderer | Use |
|---|---|---|---|
| `deck-live.html` | `slides[].live_body` | Jinja2 + agency branding | Screen-shared on the call |
| `deck-leave-behind.html` | `slides[].live_body` + `leave_behind_extension` | Jinja2 + agency branding | Post-call, denser, sent to buyer |
| `speaker-notes.md` | `slides[].speaker_notes` paired with `title` | Markdown writer | Second-screen reference for the agent |
| `deck-live.pdf` | HTML via Puppeteer/Playwright | Puppeteer | Read-only fallback |
| `deck-leave-behind.pdf` | HTML via Puppeteer/Playwright | Puppeteer | Sent to brand |
| `deck.pptx` | Via slide skill | Slide skill (user-provided) | Editable for buyer to share internally |
| `briefing-notes.md` | `briefing_notes` block | Markdown writer | Agent reads pre-call |
| `agenda.md` | `agenda` block | Markdown writer | Agent reference |
